...
{
public:
	SDriver(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
	...
	bool Create(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_thread = thread(&SDriver::MainThread, this);
...
private:
	...
	{
		((SDriver*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
	...
	double WaveData() {
		if (m_userFunction == nullptr)
			...
			return m_userFunction(m_dGlobalTime);
